							...
							<img src="data:image/png;base64,{{$student->passport}}" class="img elevation-2" alt="Passport" width="50px">
							 ...
							 <td>{{ $student->phone }}</td>
                               ...
                               <td>{{ $student->academic->mat_no }}</td>
                                ...
                                <td><a class="btn btn-default" href="{{ route('student.view',$student->id) }}"> View</a></td>
                               @else
                                    <td></td>
                                @endcan
                                @can('show', 'App\Student')
                                <td><a class="btn btn-default" href="{{ route('student.show',$student->id) }}">  Edit</a></td>
                                @else
                                    <td></td>
                                @endcan
                               ...
                               <td><a class="btn btn-info" href="{{ route('student.transcript',base64_encode($student->id)) }}" target="_blank"> <i class="fa fa-eye"></i> Transcript</a></td>
                                ...
                                @if ($student->status == 1)
                                    <td>
                                        {!! Form::open(['method' => 'patch', 'action' => 'AdminStudentsController@disable', 'id'=>'disableUForm'.$student->id]) !!}
                                ...
                                @elseif ($student->status == 2)
                                    <td>
                                        {!! Form::open(['method' => 'patch', 'action' => 'AdminStudentsController@disable', 'id'=>'enableUForm'.$student->id]) !!}
                                        {{ Form::hidden('id', $student->id) }}
