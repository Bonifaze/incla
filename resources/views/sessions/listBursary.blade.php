      ...
      <div class="container-fluid">
       ...
       @if ( $session->status == 1)
							 ...
							 {{--  <td>{{ $session->status }}</td>  --}}
								<td><a class="btn btn-warning" href="{{ route('session.editBursary',$session->id) }}"> <i class="fa fa-edit"></i> Edit </td>

							@if ($session->status == 1)
									<td>Current Session</td>
								@elseif($session->status == 0)
							 <td>
									{!! Form::open(['method' => 'Patch', 'route' => 'session.set_currentBursary', 'id'=>'setCurrentForm'.$session->id]) !!}
