...
<title>{{ $student->full_name }} Result</title>
	...
	@php $cgpa = $reg->cgpa(); @endphp  --}}
        ...
        <table width="100%" height="87" border="1" cellpadding="0" cellspacing="0">
                          <tr>
                            ...
                            <td colspan="2" align="center"><strong>LEVEL</strong>:
                             {{ $session->registered_courses1admin->last()?->level }}
                              {{--  {{ $session->registered_courses1admin->max('level') }}  --}}
                             </td>
                            <td colspan="2"><strong>SEMESTER</strong>:
                        {{--  {{ $session->registered_courses1admin->first()?->semester }}  --}}
                             FIRST
                             </td>
                          </tr>
                          <tr>
                          ...
                          @php
                            ...
                            $tgp2 = 0;
                          @endphp
                            @foreach ($session->registered_courses1admin as $result)
                            @php
      ...
      <table width="100%" border="1" cellpadding="0" cellspacing="0">
                          ...
                          <tr>
                            ...
                            <td>&nbsp;</td>
                            <td><span style="font-weight: bold">CGPA : {{ ($tgp_cgpa+$tgp1) > 0 && ($tcu_cgpa+$tc1) > 0 ? number_format(($tgp_cgpa+$tgp1)/($tcu_cgpa+$tc1), 2) : '0.00' }}</span></td>
                            <td>&nbsp; </td>
                          </tr>
...
<table width="100%" height="87" border="1" cellpadding="0" cellspacing="0">
  <tr>
    ...
    <td colspan="2" align="center"><strong>LEVEL</strong>:
     {{ $session->registered_courses2admin->last()?->level }}
     </td>
    <td colspan="2"><strong>SEMESTER</strong>:
 {{--   {{ $session->registered_courses2admin->first()?->semester }} --}}
 SECOND
     </td>
  </tr>
  <tr>
    ...
    @foreach ($session->registered_courses2admin as $result2)
    @php
