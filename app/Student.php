...
class Student extends Authenticatable
{
    ...
    public function excelCGPA()
    {
        ...
        $results = $this->results()->where('status',7)->whereHas('programCourse')->with('programCourse')->get();
        foreach ($results as $key => $result)
         {
            $hrs = $result->programCourse->hours;
    ...
    public function baselineCGPA()
    {
        ...
        //get baseline cgpa
        $hours = $this->academic->TC + $gpa->hours;
    ...
    public function programCourseResult($program_course_id)
    {
        $result = StudentResult::where('student_id',$this->id)
            ->where('program_course_id', $program_course_id)
            ->get();
        if(count($result) > 0)
        {
    ...
    public function semesterResultRemark()
    {
        ...
        $outstandings = SemesterResultOutstanding::with('course')->where('student_id',$this->id)
        ...
        {
            if($outstanding->type == 2)
            {
                $co .= $outstanding->course->code . ", ";
            }
            else if($outstanding->type == 1)
            {
